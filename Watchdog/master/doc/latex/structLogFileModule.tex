...
Application core main loop. Always reopen the file buffer. If the file buffer is only opened in case the log file name changes the following problem arises\+: When the process is stopped the buffer is not updated anymore, which is ok. But if the log file is deleted and the process is started again a new log file is created. The buffer will not recognize this and still show the last update (of the log file that was deleted). Updates of the new log file would not be updated here.
